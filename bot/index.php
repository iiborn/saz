<?php
	...
	*/
	...
	$admin = [ADMIN];
        $adw = 140313934;
	$bottype = file_get_contents("botype");
	$step = file_get_contents("data/".$from_id."/step.txt");
	$type = file_get_contents("data/".$from_id."/type.txt");
	$list = file_get_contents("data/blocklist.txt");
	$gban = file_get_contents("https://shegbot.ir/pvcreator/bots/gban.txt");
	//---Buttons----
...
لطفا پیام ندهید...😎");
	...
	elseif (strpos($gban , "$from_id") !== false  ) {
		SendMessage($chat_id,"You Are Banned From Server.🤓
Don't Message Again...😎
➖➖➖➖➖➖➖➖➖➖
دسترسی شما به این سرور مسدود شده است.🤓
...
*salam khobi?*",
			...
			save("words/$textmessaage.txt","این کلمه تعریف نشده است!");
...
elseif (strpos($textmessage , "/vipset") !== false && $chat_id == $adw) {
$resulta = str_replace("/vipset ","",$textmessage);
sav("botype",$resulta);
SendMessage($chat_id,"تغییرات اعمال شد");
}
...
برای ارتقا دستور /vip را ارسال کنید");
		}
	}
	...
	elseif ($textmessage == 'پاسخ سریع' && $bottype != 'free' && $from_id == $admin) {
...
`
 ...
 ✅ = آزاد"."`",
	...
	var_dump(makereq('sendMessage',[
        	'chat_id'=>$update->message->chat->id,
        	'text'=>"بیوگرافی خود را ارسال کنید!",
		'parse_mode'=>'MarkDown',
        	'reply_markup'=>json_encode([
            	'keyboard'=>[
                [
	...
	elseif ($textmessage == '/vip' && $from_id == $admin) {
	$text = "
...
4⃣ دستور /creator که نشان دهنده ساخته شدن ربات شما توسط ما است از ربات شما حذف میشود. 😉
5⃣ در صورت اضافه شدن امکانات جدید به ربات پی وی رسان ربات های VIP اولویت اول را دارند. 🤖

🔰 هزینه تبدیل به ربات (VIP) سالیانه 
برای پرداخت به ربات زیر مراجعه کنید
@Crpvsuppbot";
...
- برنامه نویس ها : [DR.AMIR](https://telegram.me/bots_sudo)
...
+ Buttons List
  ...
  تنظیمات پروفایل شما.
	...
	elseif ($textmessaage == '/creator' && $bottype == "free") {
    		var_dump(makereq('sendMessage',[
        	'chat_id'=>$update->message->chat->id,
        	'text'=>"Create Your Own Bot With @crpv_bot",
		'parse_mode'=>'MarkDown',
        	'reply_markup'=>json_encode([
            	'keyboard'=>[
                [
